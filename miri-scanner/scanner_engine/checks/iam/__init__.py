from . import cis_2_4_root_no_access_key
from . import cis_2_5_root_mfa
...
from . import cis_2_13_group_only_permissions
from . import cis_2_14_no_admin_policy
...
def run_all_iam_checks(iam_client):
    ...
    findings.append(cis_2_4_root_no_access_key.run(iam_client))
    findings.append(cis_2_5_root_mfa.run(iam_client))
    findings.append(cis_2_8_password_length_complexity.run(iam_client))
    findings.append(cis_2_9_password_reuse.run(iam_client))
    findings.append(cis_2_14_no_admin_policy.run(iam_client))
    ...
    for page in paginator.paginate():
        for user in page['Users']:
            ...
            # 2.13 IAM users receive permissions only through groups 체크
            findings.append(cis_2_13_group_only_permissions.run(iam_client, user))
